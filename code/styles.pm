...
sub brewcolor {
  ...
  my @drinkcolors = (   # color, pattern. First match counts, so order matters
    "003000", "restaurant|bar|night|meal|producer|home|hall|church|null", # regular bg color, no highlight
    "808080", "adjustment", # gray for payment adjustments
    "eac4a6", "wine[, ]+white",
    "801414", "wine[, ]+red",
    "4f1717", "wine[, ]+port",
    "aa7e7e", "wine",
    "f2f21f", "Pils|Lager|Keller|Bock|Helle|IPL|Kölsch",
    "e5bc27", "vienna",
    "7A5C33", "Dark|dunkel|schwarz|juleb|Classic|Altbi|alt",
  ...
  if ( ! $warned{$type} ) {  # Not already warned about this type
    my $logline = $line;
    my $trace = "";
    $trace = longmess("  trace") unless ($line);
